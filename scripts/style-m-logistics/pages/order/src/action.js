...
var baseUrl = 'https://56.wuage.com/logistics/web';
var categorys = [];
new Vue({
    ...
    methods: {
        back() {
            location.href = 'https://m.wuage.com';
        },
        getCarriers(){
            //获取供应商
            this.$http.jsonp(baseUrl + '/carriers', {
                params: {
                    'notcarriers': 1
                }
        ...
        getMemberId(){
            this.$http.jsonp('https://login.wuage.com/getLoginInfo', {
                'jsonp': 'jsonpCallback'
            }).then(function (res) {
                var data = res.body;
                if (data.logined == 'N') {//未登录
                    wuage.openModel({
                        content: '需要先登录',
                        showCancel: false,
                        confirmText: '确定',
                        confirm: function () {
                            location.href = 'https://m.wuage.com/member/login-page?url=' + location.href;
                        }
                    });
                } else {//已登录
                    this.memberId = data.m_id;
                    this.checkuser();
                }
            }, function () {
        ...
        checkuser(){
            //获取用户是否已授权
            this.$http.jsonp(baseUrl + '/checkuser', {
                params: {
                    'm_id': this.memberId
                }
            }).then(function (res) {
        ...
        agree(){
            //用户授权接口
            var _this = this;
            this.$http.jsonp(baseUrl + '/useradd', {
                params: {
                    'm_id': this.memberId
                }
            }).then(function (res) {
        ...
        sendRequest(){
            ...
            var _this = this;
            this.$http.get('/logistics/save.jsonp', {
                emulateJSON: true,
                params: {
                    channel: channel,
                ...
                var data = response.body
                if (data.messageId === 'ok') {
                    location.href = '/logistics/success?categoryId=' + _this.categoryId
                } else {
